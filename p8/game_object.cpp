...
Status Game_object::get_member_variable_type(string name, Gpl_type &type)
{
  ...
  {
    type = NO_TYPE; // just in case the caller is using this type
...
Status Game_object::set_member_variable(string name, int value)
{
  ...
  if (variable->m_derived)
    Error::error(Error::CANNOT_CHANGE_DERIVED_ATTRIBUTE,
          name, gpl_type_to_string(m_object_type));
...
void Game_object::print(string symbol_name, ostream &os) const
{
  assert(m_object_type == CIRCLE
         || m_object_type == RECTANGLE
         || m_object_type == TRIANGLE
         || m_object_type == TEXTBOX
         || m_object_type == PIXMAP
        );

  os << gpl_type_to_string(m_object_type) << " " << symbol_name << endl << "{" << endl;
