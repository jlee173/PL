...
initialization_block:
    T_INITIALIZATION statement_block
      ...
      static Event_manager *event_manager = Event_manager::instance();
			event_manager->insert($2, Window::INITIALIZE);
		}
    ;
...
termination_block:
    T_TERMINATION statement_block
		{
      static Event_manager *event_manager = Event_manager::instance();
			event_manager->insert($2, Window::TERMINATE);
		}
    ;
    ...
    T_ANIMATION T_ID T_LPAREN check_animation_parameter 
		{
			...
			static Symbol_table *symbol_table = Symbol_table::instance();
			Symbol *sym = symbol_table->lookup(*$2);
			if(symbol_table->lookup(*$2) != NULL)
			{
				there_is_forward = true;
				Animation_block* my_ani = sym->get_animation_block_value();
				m_stack.push(my_ani);
				Game_object *g_obj = $4->get_game_object_value();
				my_ani->execute(g_obj);
			}
			else
			{
				there_is_forward = false;
